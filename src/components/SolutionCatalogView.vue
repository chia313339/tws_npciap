...
const buildContactRow = ({ label, value, href = '', linkLabel = '' }) => {
  ...
  const externalIcon = isExternal ? '<span class="external-hint">（另開新視窗）</span>' : ''
  const content = href
...
const openSolutionModal = (card) => {
  ...
  const manualButton = manualUrl
    ? `<a class="solution-modal-manual-btn" href="${escapeHtml(manualUrl)}" target="_blank" rel="noopener noreferrer" aria-label="${escapeHtml(manualButtonLabel)}" title="${escapeHtml(manualButtonLabel)}">點擊看「操作說明」<span class="external-hint">（另開新視窗）</span></a>`
