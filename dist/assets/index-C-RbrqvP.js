...
`,"<br />"),i=c=>c.solutionName||c.vendorName||c.companyName||c.cardTitle||c.modalData?.name||"方案",a=c=>c.companyName||c.modalData?.companyShortName||c.modalData?.company||"",r=c=>{const d=i(c),f=a(c);return f?`查看 ${f}「${d}」方案詳細資訊`:`查看「${d}」詳細資訊`},l=({label:c,value:d,href:f="",linkLabel:h=""})=>{const v=String(d||"").trim();if(!v)return"";const C=/^https?:\/\//i.test(f),P=C?' target="_blank" rel="noopener noreferrer"':"",y=C?'<i class="fa-solid fa-arrow-up-right-from-square external-link-icon" aria-hidden="true"></i>':"",E=f?`<a class="solution-modal-contact-link" href="${t(f)}"${P} aria-label="${t(h||`${c}：${v}`)}" title="${t(h||`${c}：${v}`)}">${t(v)}${y}</a>`:`<span class="solution-modal-contact-value">${t(v)}</span>`;return`
  ...
  `},u=c=>{const d=c.modalData||c,f=d.name||c.solutionName||c.companyName||c.cardTitle||c.vendorName||"方案詳細資訊",h=n(d.manualUrl),v=n(d.websiteUrl),C=s(d.contactEmail),P=d.detailImage||d.image||c.logo||"",y=`${f} 示意圖`,E=`開啟「${f}」操作說明（另開新視窗）`,S=h?`<a class="solution-modal-manual-btn" href="${t(h)}" target="_blank" rel="noopener noreferrer" aria-label="${t(E)}" title="${t(E)}">點擊看「操作說明」<i class="fa-solid fa-arrow-up-right-from-square" aria-hidden="true"></i></a>`:'<span class="solution-modal-manual-btn solution-modal-manual-btn--disabled" aria-disabled="true">尚無操作說明<i class="fa-solid fa-arrow-pointer" aria-hidden="true"></i></span>',b=[l({label:"聯絡人",value:d.contactPerson}),l({label:"公司聯絡電話",value:d.companyPhone}),l({label:"聯絡信箱",value:d.contactEmail,href:C,linkLabel:`寄信給 ${d.contactPerson||d.company||"方案聯絡窗口"}：${d.contactEmail}`}),l({label:"公司官網網址",value:d.websiteUrl,href:v,linkLabel:`前往 ${d.company||c.companyName||"公司"} 官網（另開新視窗）`})].filter(Boolean).join(""),T=`
